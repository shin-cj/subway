...
public class SubwayTopview extends JPanel {
	...
	public SubwayTopview(Color lineColor, String startSt, String endSt, List<String> stationData) {
	    ...
	    for (int i = 0; i < lineStations.length; i++) {
	        if (lineStations[i].equals(startSt)) startIdx = i;
	        if (lineStations[i].equals(endSt)) endIdx = i;
	    }
	        ...
	        for (int i = 0; i < 6; i++) {
	            for (int j = 0; j < 7; j++) {
	                if (rand.nextDouble() < 0.3) {
	                    int offset = (rand.nextInt(5) + 1) * direction; // 방향 반영
	...
	private void setupUI(Color lineColor, String startSt, String endSt, String[] stationData) {
		...
		for (int i = 0; i < seatPanels.length; i++) {
			...
			for (int j = 0; j < 7; j++) {
				...
				final int col = j;
				// 좌석 이름 예시: "L1-1", "L1-2" ...
				String seatName = (i < 3 ? "L" + (i + 1) : "R" + (i - 2)) + "-" + (j + 1);
		...
		JPanel subColorBg = new JPanel(new GridBagLayout());
		subColorBg.setBackground(new Color(245, 245, 245));// 배경색
		...
		scrollPane.setBounds(0, 60, 426, 531); // 상자의 크기 결정
