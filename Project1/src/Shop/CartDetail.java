...
public class CartDetail extends JFrame {
	public CartDetail(ShopDataManager dataManager, Runnable updateMainUI) {
		...
		// 💡 [추가 1] 총 금액을 보여줄 라벨 생성
		JLabel totalLabel = new JLabel("총 합계: 0 P");
		totalLabel.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		totalLabel.setForeground(new Color(0, 102, 204)); // 예쁜 파란색
		totalLabel.setHorizontalAlignment(SwingConstants.RIGHT);
		totalLabel.setBorder(BorderFactory.createEmptyBorder(5, 0, 0, 0)); // 상단 여백

		// 💡 [추가 2] '전체 구매' 버튼과 '총 합계' 라벨을 위아래로 묶어주는 우측 패널
		...
		topPanel.add(rightTopPanel, BorderLayout.EAST); // 기존 버튼 대신 패널 부착
		...
		// 💡 [추가 3] 장바구니 총 금액을 실시간으로 계산해서 라벨에 적어주는 마법의 기능!
		...
		updateTotalLabel.run(); // 창 켜질 때 최초 1회 계산해서 띄우기
			...
			if (currentPrice >= totalPrice) {
				...
				if (UserInfo.currentUser != null) {
					...
					UserInfo.currentUser
							.addPointHistory("[" + timeStr + "전체 구매] 총 " + totalQty + "개 결제: -" + totalPrice + " P");
				}

				for (Map.Entry<String, Integer> entry : cart.entrySet()) {
					String itemName = entry.getKey();
					int qty = entry.getValue();
					dataManager.getOrderHistory().put(itemName,
							dataManager.getOrderHistory().getOrDefault(itemName, 0) + qty);
				}

				cart.clear();

				String msg = String.format("전체 구매가 완료되었습니다! \n\n총 구매 수량 : %d개\n총 결제 금액 : %dP\n남은 잔액 : %dP", totalQty,
						totalPrice, dataManager.getMyPrice());
			...
			buyBtn.addActionListener(e -> {
				...
				if (currentPrice >= totalPrice) {
					...
					if (UserInfo.currentUser != null) {
						String timeStr = java.time.LocalDateTime.now()
								.format(java.time.format.DateTimeFormatter.ofPattern("MM/dd HH:mm"));
						UserInfo.currentUser.addPointHistory(
								"[" + timeStr + "] [" + itemName + "] " + qty + "개 구매: -" + totalPrice + " P");
					}
